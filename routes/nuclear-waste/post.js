...
router.get('/:post_id/statistics', function (req, res) {
    req.db.query('SELECT * FROM `statistic` WHERE `post_id`=?' , req.params.post_id,
    function (error, results) {
        if(error){
            console.log(error);
            res.status(HttpStatus.INTERNAL_SERVER_ERROR);
...
router.post('/:post_id/statistics', function (req, res) {
    req.body.post_id = req.params.post_id;
    req.db.query('INSERT INTO `statistic` SET ?', req.body,
        function(error, result){
            if(error){
                console.log(error);
                res.status(HttpStatus.INTERNAL_SERVER_ERROR);
                ...
                req.db.query('SELECT * FROM `statistic` WHERE `id`=?', result.insertId,
                    function (error, results) {
                        if(error){
                            console.log(error);
                            res.status(HttpStatus.INTERNAL_SERVER_ERROR);
...
router.put('/:post_id/statistics/:id', function (req, res) {
    req.db.query('UPDATE `statistic` SET `count` = `count` + 1 WHERE `id` = ?', [req.params.id],
    function (error, results) {
        if(error){
            console.log(error);
            res.status(HttpStatus.INTERNAL_SERVER_ERROR);
            ...
            req.db.query('SELECT * FROM `statistic` WHERE `id` = ?', [req.params.id],
            function (error, results) {
                if(error){
                    console.log(error);
                    res.status(HttpStatus.INTERNAL_SERVER_ERROR);
